...
%%
%TX
[num,txt,data]=xlsread('E:\Preliminary classification data.xls','TXrenewable');
%数量
...
%title('Texas renewable resource energy change with annual variation (quantity),1-6 represent EMTCP, ENACP, ENCCP, ENICP, ENPRP, ENTCP')
axis square
grid off
%价格
...
%title('Texas renewable resource energy change with annual variation (price),1-4 represent EMACV，EMCCV，EMICV，EMTCV')
axis square
grid off
...
%title('Texas renewable resource energy change with annual variation (energe)')
axis square
grid off
...
xlabel('year')
ylabel('Billion Btu')
%title('Texas renewable resource energy change with annual variation（Total output and total consumption）')
axis square
grid off
